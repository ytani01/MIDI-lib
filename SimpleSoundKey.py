...
class SampleApp:
    ...
    def __init__(self, midi_file, debug=False):
        ...
        self._cui = CuiUtil.CuiKey([
            ...
            CuiUtil.CuiCmd(['KEY_UP', 'KEY_DOWN'], self.up_down),
            CuiUtil.CuiCmd(['KEY_LEFT', 'KEY_RIGHT'], self.change_sec),
    ...
    def change_sec(self, key_sym):
        ...
        print('sec=%.1f' % self._sec)

    def up_down(self, key_sym):
        """
        """
        self.__log.debug('key_sym=%a', key_sym)
    ...
    def play(self, key_sym):
        ...
        print('%.2fHz, %ssec, vol:%s, rate:%s' % (
            freq, self._sec, self._vol, self._rate))
